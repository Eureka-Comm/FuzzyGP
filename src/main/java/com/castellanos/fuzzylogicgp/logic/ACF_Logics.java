...
class MathOper{
     ...
     *
     * @param   v1   a value
     * @return  the value ln&nbsp;{@code v1}, the natural logarithm of
     *          ({@code v1} + square root {@code 1 + v1^2}).
     */
    ...
    /**
     * Returns the valuo of logic of a {@code double}
     * value. Special cases:
     * <ul><li>If the argument is 1, then the result is -1 * the natural logarithm (base <i>e</i>) of v1.
     * <li>If the argument is 2, then the result is -1 * log({@code v1}^m)
     * <li> If the argument is 3, then the result is -1 * {@code asinh}(log({@code v1})).
...
public class ACF_Logics implements Logic {

    private Integer L,m;
    private MathOper Mo = new MathOper();
    public ACF_Logics(){}
    
    public ACF_Logics(int L, int m){
        this.L = L;
        this.m = m;
    }
